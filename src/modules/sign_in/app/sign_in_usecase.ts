...
import { ForbiddenAction, UserNotConfirmed, UserSignUpNotFinished } from "src/shared/helpers/errors/usecase_errors";
...
export class SignInUseCase {
  ...
  async execute(indentifier: string, password: string, isWeb: boolean) {
    let email = User.validateEmail(indentifier) ? indentifier : null;
  ...
  if (session.accessToken && emailRegex.test(user?.userUsername as string)) throw new UserSignUpNotFinished()
