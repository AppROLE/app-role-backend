...
export class CognitoStack extends Construct {
  ...
  constructor(scope: Construct, id: string) {
    ...
    this.userPool = new cognito.UserPool(
      this,
      `${Environments.STACK_NAME}-UserPool-${stage}`,
      {
        ...
        userVerification: {
          ...
          emailBody:
            "Hello {username}, Thanks for signing up to AppROLE! Your verification code is {####}",
        ...
        customAttributes: {
          ...
          roleType: new cognito.StringAttribute({
            minLen: 1,
            maxLen: 2048,
            mutable: true,
          }),
          acceptedTerms: new cognito.BooleanAttribute({ mutable: true }),
          confirmationCode: new cognito.StringAttribute({
            minLen: 1,
            maxLen: 2048,
            mutable: true,
          }),
    ...
    this.client = this.userPool.addClient(
      `${Environments.STACK_NAME}-UserPoolClient-${stage}`,
      {
        userPoolClientName: `${Environments.STACK_NAME}-UserPoolClient-${stage}`,
